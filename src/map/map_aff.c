...
static void help_aff(sfRenderWindow *window, map_t *map)
{
	sfRectangleShape *rect = sfRectangleShape_create();
	sfVector2u window_size = sfRenderWindow_getSize(window);
	int d = (window_size.x / map->size) / 30;
	int d2 = window_size.x - window_size.x / map->size * map->size;
	sfVector2f size = {window_size.x / map->size - d, window_size.y / map->size - d};
	sfVector2f pos;
	...
	if (for_fun == 0) {
		...
		for (size_t i = 0; i < 4; i++) {
			sfRectangleShape_rotate(rect, angle + 45);
			sfRenderWindow_drawRectangleShape(window, rect, NULL);
		}
	else
		for_fun = rand() % 3;
